...
describe("parse", () => {
  for (let [code, tree] of [
    ...
    ["-1 + 2", ["+", "-1", "2"]],
    ["2 + -1", ["+", "2", "-1"]],
    ["trunc(2018-12-01T02:34:56)", ["trunc", "2018-12-01T02:34:56"]]
  ]) {
    it(`parses "${code}"`, () => {
  ...
  describe("operator precedence", () => {
    for (let [higher, lower] of [
      ...
      it(`${higher} binds tighter than ${lower}`, () => {
        assert.deepStrictEqual(parse(`a ${lower} b ${higher} c`), [
          lower,
          "a",
          [higher, "b", "c"]
        ]);
        assert.deepStrictEqual(parse(`a ${higher} b ${lower} c`), [
          lower,
          [higher, "a", "b"],
          "c"
        ]);
    ...
    for (let [fst, snd] of [["+", "-"], ["-", "+"], ["*", "/"], ["/", "*"]]) {
      it(`${fst} binds equally with ${snd}`, () => {
        assert.deepStrictEqual(parse(`a ${fst} b ${snd} c ${fst} d`), [
          fst,
          [snd, [fst, "a", "b"], "c"],
          "d"
        ]);
